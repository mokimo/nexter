...
async function isFileComplete(endpoint, projectId, code, basePath) {
  // if (summaryReport) return false;

  const opts = { headers: { 'Content-Type': 'application/json' } };
  opts.headers.Authorization = `Bearer ${token}`;

  const url = `${endpoint}/files-api/v2/projects/${projectId}/locales/${code}/file/status?fileUri=${basePath}`;
  const resp = await fetch(url, opts);
  const json = await resp.json();
  const { completedWordCount, totalWordCount } = json.response.data;
  return completedWordCount === totalWordCount;
}
...
export async function getStatusAll(title, config, langs, urls, actions) {
  ...
  await Promise.all(langs.map(async (lang) => {
    const completed = [];
    for (const url of urls) {
      const complete = await isFileComplete(endpoint, projectId, lang.code, url.basePath);
      console.log(lang.code, url.basePath, complete);
      if (complete) completed.push(complete);
    }
    console.log(completed);
    lang.translation.translated = completed.length;
  }));
  ...
  //     const { data } = response;
  //     const { contentProgressReport } = data;
  //     // Loop through the languages of the URL in the job.
  //     const urlStatuses = await Promise.all(contentProgressReport.map(async (report) => {
  //       const { targetLocaleId, summaryReport } = report;
  //       const lang = langs.find((daLang) => daLang.code === targetLocaleId);
  //       // eslint-disable-next-line max-len
  //       const complete = await isFileComplete(summaryReport, endpoint, projectId, targetLocaleId, url.basePath);
  //       if (complete) lang.translation.translated += 1;
  //       return complete;
  //     }));
  //     const lang = langs.find((daLang) => daLang.code === targetLocaleId);
  //   }
