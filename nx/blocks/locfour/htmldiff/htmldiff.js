...
export function addId(block) {
  if (block.className?.toLowerCase().startsWith('block-group-end')) return;

  if (!block.dataset.id) {
    const hash = objectHash(normalizeHTML(block.innerHTML));
    block.dataset.id = `${hash.substring(0, HASH_LENGTH)}`;
  }
}

export function addIds(doc) {
  const topLevelEls = doc.querySelectorAll('main > div > *');
  topLevelEls.forEach(addId);
}
...
export function getBlockMap2(dom) {
  const blocks = [...dom.querySelectorAll('main > div > div')];
  const blockMap = blocks.reduce(
    (map, block) => {
      if (!block.dataset.id) {
        addId(block);
      }
      map.set(block.dataset.id, block);
      return map;
    },
    new Map(),
  );
  ...
  const sectionMap = sections.reduce((acc, section) => {
    const sectionBlocks = [...section.children]
      .filter(isNotEmptyParagraphEl);
    const sectionArr = sectionBlocks.map((block) => {
      const ids = [];
      if (block.dataset.id) {
        ids.push(block.dataset.id);
      } else {
        // block not a table but top level content
        const stringToHash = normalizeHTML(block.outerHTML || blockGroupToStr(block));
        const hash = objectHash(stringToHash);
        contentBlocks.set(hash, block);
        ids.push(hash);
      }
      return ids;
    });
  ...
  return { blockMap, sectionMap, contentBlocks };
}

function getDiffArray(originalDOM, modifiedDOM) {
  const original = getBlockMap2(originalDOM);
...
export async function diffHtml(original, modified) {
  const diff = getDiffArray(original, modified);
